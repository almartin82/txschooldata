...
- [ ] `pytest tests/test_pytxschooldata.py` — all tests pass
- [ ] `pkgdown::build_site()` — builds without errors
- [ ] Vignettes render (no `eval=FALSE` hacks)
- [ ] All new exported functions added to `_pkgdown.yml` reference index
...
When you add functions with `@export` roxygen tags, pkgdown requires them to be listed in `_pkgdown.yml` under the `reference:` section. If they're missing, pkgdown CI fails with:
...
If a function is internal/helper (not user-facing), add `@keywords internal` to exclude it from documentation:
...
1. Write function with `@export` tag
2. Run `devtools::document()` to regenerate `.Rd` files
3. Add function to appropriate section in `_pkgdown.yml`
4. Run `pkgdown::build_site()` to verify build works
5. Commit both code changes and `_pkgdown.yml` updates together

**Lessons learned:**
- Graduation rate PR (#10) failed CI for this reason
- STAAR PR (#11) initially failed for the same reason
- Local `pkgdown::build_site()` testing catches this before pushing
- This is a systematic workflow issue, not a one-time mistake
