...
class TestFetchEnrMulti:
    ...
    def test_returns_dataframe(self):
        """Returns a pandas DataFrame."""
        import pytxschooldata as tx
        df = tx.fetch_enr_multi([2023, 2024])
        assert isinstance(df, pd.DataFrame)
    ...
    def test_more_rows_than_single_year(self):
        ...
        import pytxschooldata as tx
        df_single = tx.fetch_enr(2024)
        df_multi = tx.fetch_enr_multi([2023, 2024])
...
class TestDataIntegrity:
    ...
    def test_consistent_between_single_and_multi(self):
        ...
        import pytxschooldata as tx
        df_single = tx.fetch_enr(2024)
        df_multi = tx.fetch_enr_multi([2024])
