# ==============================================================================
# LIVE Pipeline Tests for txschooldata
# ==============================================================================
#
...
test_that("Texas DOE website is accessible", {

  skip_if_offline()
  skip("TODO: Add Texas DOE URL and verify HTTP 200")
})
...
test_that("Can download Texas enrollment data file", {
  skip_if_offline()
...
test_that("Can parse Texas enrollment file", {
  skip_if_offline()
...
test_that("txschooldata data file has expected columns", {
...
test_that("get_raw_enr returns data for valid year", {
  ...
  # Get available years
  years_info <- txschooldata::get_available_years()
  ...
  tryCatch({
    raw <- txschooldata:::get_raw_enr(test_year)
...
test_that("get_available_years returns valid year range", {
  result <- txschooldata::get_available_years()
...
test_that("fetch_enr returns data with no Inf or NaN", {
  ...
  tryCatch({
    ...
    data <- txschooldata::fetch_enr(test_year, tidy = TRUE)
...
test_that("Enrollment counts are non-negative", {
  ...
  tryCatch({
    ...
    data <- txschooldata::fetch_enr(test_year, tidy = FALSE)
...
test_that("tidy=TRUE and tidy=FALSE return consistent totals", {
  ...
  tryCatch({
    ...
    wide <- txschooldata::fetch_enr(test_year, tidy = FALSE)
    tidy <- txschooldata::fetch_enr(test_year, tidy = TRUE)
...
test_that("Cache functions exist and work", {
  ...
  tryCatch({
    path <- txschooldata:::get_cache_path(2024, "enrollment")
